...
func TestQueryNodes_Method(t *testing.T) {
	...
	result, err := g.QueryNodes(NodeMethod)
	if err != nil {
...
func TestQueryNodes_EmptyResult(t *testing.T) {
	...
	result, err := g.QueryNodes(NodeCall)
	if err != nil {
...
func TestQueryNodesByFile_RejectsEmptyPath(t *testing.T) {
	...
	_, err := g.QueryNodesByFile("", NodeMethod)
	if err == nil {
...
func TestTaintPaths_RejectsEmptySources(t *testing.T) {
	...
	_, err := g.TaintPaths(nil, []TaintSink{{NodeID: "1", Kind: SinkSQL}})
	if err == nil {
...
func TestTaintPaths_RejectsEmptySinks(t *testing.T) {
	...
	_, err := g.TaintPaths([]TaintSource{{NodeID: "1"}}, nil)
	if err == nil {
...
func TestTaintPaths_ParsesFindings(t *testing.T) {
	...
	paths, err := g.TaintPaths(
		[]TaintSource{{NodeID: "src"}},
		[]TaintSink{{NodeID: "snk", Kind: SinkSQL}},
	)
	if err != nil {
...
func TestTaintPaths_CapsAtMaxTaintPaths(t *testing.T) {
	...
	paths, err := g.TaintPaths(
		[]TaintSource{{NodeID: "s"}},
		[]TaintSink{{NodeID: "k", Kind: SinkSQL}},
	)
	if err != nil {
		t.Fatalf("TaintPaths: %v", err)
	}
...
func TestPreFlagSinks_JavaSQLSink(t *testing.T) {
	...
	if sinks[0].Kind != SinkSQL {
		t.Errorf("sink.Kind = %v, want %v", sinks[0].Kind, SinkSQL)
	}
	if sinks[0].Line != 1 {
...
func TestPreFlagSinks_PythonEval(t *testing.T) {
	...
	if sinks[0].Kind != SinkEval {
		t.Errorf("sink.Kind = %v, want %v", sinks[0].Kind, SinkEval)
	}
	if sinks[0].Line != 2 {
...
func TestGraphPreFlaggedSinks(t *testing.T) {
	...
	c.preFlaggedMu.Lock()
	c.preFlaggedSinks = []TaintSink{
		{Kind: SinkSQL, File: "test.java", Line: 10},
	}
...
func TestTaintPathToFinding_SetsMatchedCode(t *testing.T) {
	...
	path := TaintPath{
		Source: TaintSource{NodeID: "1", Kind: "http_param", File: f, Line: 1},
		Sink:   TaintSink{NodeID: "2", Kind: SinkSQL, File: f, Line: 3},
	}
...
func TestTaintPathToFinding_ConfidenceMapsToSeverity(t *testing.T) {
	path := TaintPath{
		Source: TaintSource{NodeID: "1", Kind: "http_param", File: "f.java", Line: 1},
		Sink:   TaintSink{NodeID: "2", Kind: SinkSQL, File: "f.java", Line: 3},
	}
...
func TestTaintPathToFinding_SSVCAutomatable(t *testing.T) {
	tests := []struct {
		...
		{SinkEval, "Yes"},
		{SinkFileWrite, "No"},
		{SinkDeserialization, "No"},
	}
	for _, tc := range tests {
		path := TaintPath{
			Source: TaintSource{NodeID: "1", Kind: "http_param", File: "f.java", Line: 1},
			Sink:   TaintSink{NodeID: "2", Kind: tc.kind, File: "f.java", Line: 3},
		}
		f := TaintPathToFinding(path, LanguageJava)
...
func TestTaintPathToFinding_SSVCTechnicalImpact(t *testing.T) {
	tests := []struct {
		...
		{SinkCommand, "Total"},
		{SinkEval, "Total"},
		{SinkDeserialization, "Total"},
		{SinkFileWrite, "Partial"},
		{SinkTemplate, "Partial"},
	}
	for _, tc := range tests {
		path := TaintPath{
			Source: TaintSource{NodeID: "1", Kind: "http_param", File: "f.java", Line: 1},
...
func TestTaintPathToFinding_PoeContextPopulated(t *testing.T) {
	path := TaintPath{
		Source: TaintSource{NodeID: "src1", Kind: "http_param", File: "f.java", Line: 1},
		Sink:   TaintSink{NodeID: "snk2", Kind: SinkSQL, File: "f.java", Line: 5},
