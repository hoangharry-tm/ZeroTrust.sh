...
func TestIntegration_TaintPath_SQLInjection(t *testing.T) {
	...
	methods, err := g.QueryNodesByFile("MainController.java", NodeMethod)
	if err != nil {
	...
	paths, err := g.TaintPaths(
		[]TaintSource{{NodeID: getUserID, Kind: "method_parameter", File: "MainController.java"}},
		[]TaintSink{{NodeID: getUserID, Kind: SinkSQL, File: "MainController.java"}},
	)
	if err != nil {
...
func TestIntegration_TaintFlowsQuery(t *testing.T) {
	...
	methods, err := c.Graph().QueryNodes(NodeMethod)
	if err != nil {
		t.Fatalf("QueryNodes: %v", err)
	}
	var getUserID string
...
func TestIntegration_SaveLoadCPG(t *testing.T) {
	...
	liveMethods, err := c.Graph().QueryNodes(NodeMethod)
	if err != nil {
	...
	loadedMethods, err := c2.Graph().QueryNodes(NodeMethod)
	if err != nil {
...
func TestIntegration_IncrementalPatch(t *testing.T) {
	...
	afterMethods, err := g.QueryNodes(NodeMethod)
	if err != nil {
...
func TestIntegration_PathA_E2E(t *testing.T) {
	...
	for _, f := range javaFiles {
		calls, err := g.QueryNodesByFile(f, NodeCall)
		if err != nil {
			continue
		}
...
func TestIntegration_IngestCPGToSQLite_Parity(t *testing.T) {
	...
	httpMethods, err := g.QueryNodes(NodeMethod)
	if err != nil {
	...
	sqlMethods, err := g2.QueryNodes(NodeMethod)
	if err != nil {
