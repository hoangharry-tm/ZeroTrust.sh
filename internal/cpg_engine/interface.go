...
// Package cpg_engine wraps the Joern CPG engine HTTP API (Apache 2.0) and
// exposes the Code Property Graph as a small, provider-agnostic interface
// ([Graph]). See joern.go for the full package documentation (architecture,
// subprocess lifecycle, security contract, incremental patching).
//
// This file defines the public surface: construct a [Client] with [New],
// build a graph with [Client.BuildCPG] or [Client.LoadCPG], then call
// [Client.Graph] to get a [Graph] for querying. Everything else — the Joern
// DSL query builders, the HTTP wire protocol, JSON parsing — is unexported.
package cpg_engine

// NodeType classifies a CPG vertex by its structural role.
type NodeType string

// NodeType constants for CPG vertex classification.
const (
...
// EdgeType constants for CPG directed-relationship classification.
const (
...
// SinkKind categorises a dangerous sink node found during pre-flagging.
// Tree-sitter pre-flags these before the CPG build so they are always in scope.
