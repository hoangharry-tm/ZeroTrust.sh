...
// tempSQLiteDB opens a fresh SQLite DB in a temp directory and returns the DB.
func tempSQLiteDB(t *testing.T) *sqlite.DB {
	t.Helper()
	dir := t.TempDir()
...
func TestSetSQLiteBackend_PropagatesToNewGraphs(t *testing.T) {
	db := tempSQLiteDB(t)
	...
	// Graphs created before SetSQLiteBackend — reads from Client, so nil
	g0 := c.newGraph(context.Background())
	if g0.sqliteDB() != nil {
		t.Error("expected nil db from Client before SetSQLiteBackend")
	}

	// Set backend
	c.SetSQLiteBackend(db, "proj-test", "v1")
	...
	if g0.sqliteDB() == nil {
		t.Fatal("expected g0 to see backend via Client after SetSQLiteBackend")
	}
	if g0.sqliteProjectID() != "proj-test" {
		t.Errorf("g0 projectID: got %q, want %q", g0.sqliteProjectID(), "proj-test")
	}
	if g0.sqliteCPGVersion() != "v1" {
		t.Errorf("g0 cpgVersion: got %q, want %q", g0.sqliteCPGVersion(), "v1")
	}

	// g1 (created after) should also see the backend.
	g1 := c.newGraph(context.Background())
	if g1.sqliteDB() == nil {
		t.Fatal("expected non-nil db after SetSQLiteBackend")
	}
	if g1.sqliteProjectID() != "proj-test" {
		t.Errorf("g1 projectID: got %q, want %q", g1.sqliteProjectID(), "proj-test")
	}
	if g1.sqliteCPGVersion() != "v1" {
		t.Errorf("g1 cpgVersion: got %q, want %q", g1.sqliteCPGVersion(), "v1")
	}
}

func TestQueryNodes_ReadsFromSQLite(t *testing.T) {
	db := tempSQLiteDB(t)
	...
	// Mock server that should NOT be called (SQLite has data)
	httpCalls := 0
	srv := mockServer(t, func(q string) (string, bool) {
		httpCalls++
		return "[]", true
	...
	c := newTestClient(t, srv)
	c.SetSQLiteBackend(db, proj, ver)
	g := c.GraphWithContext(ctx)

	nodes, err := g.QueryNodes(cpg.NodeMethod)
	if err != nil {
		t.Fatalf("QueryNodes: %v", err)
	}
	if httpCalls > 0 {
		t.Errorf("expected 0 HTTP calls when SQLite has data, got %d", httpCalls)
	}
	if len(nodes) != 3 {
...
func TestQueryNodes_ReturnsEmptyFromSQLite(t *testing.T) {
	db := tempSQLiteDB(t)
	ctx := context.Background()
	proj := "proj-empty"
	ver := "v1"

	// SQLite has no nodes — should return empty, NOT fall back to HTTP
	...
	select {
	case <-httpCalled:
		t.Error("SQLite returned empty — should not have fallen back to HTTP")
	default:
	}
	if len(nodes) != 0 {
		t.Errorf("expected 0 nodes from empty SQLite, got %d", len(nodes))
	}
}

func TestGetCallers_ReadsFromSQLite(t *testing.T) {
	db := tempSQLiteDB(t)
	ctx := context.Background()
	proj := "proj-callers"
	ver := "v1"
...
func TestGetCallees_ReadsFromSQLite(t *testing.T) {
	db := tempSQLiteDB(t)
	ctx := context.Background()
	proj := "proj-callees"
	ver := "v1"
...
func TestGetCallGraph_FromSQLite(t *testing.T) {
	db := tempSQLiteDB(t)
	ctx := context.Background()
	proj := "proj-cg"
	ver := "v1"
...
func TestGetNeighboursAtDepth_FromSQLiteUsesRecursiveCTE(t *testing.T) {
	db := tempSQLiteDB(t)
	ctx := context.Background()
	proj := "proj-bfs"
	ver := "v1"
...
func TestQueryNodesByFile_FromSQLite(t *testing.T) {
	db := tempSQLiteDB(t)
	ctx := context.Background()
	proj := "proj-file"
	ver := "v1"
	...
	nodes, err := g.QueryNodesByFile("server.go", cpg.NodeMethod)
	if err != nil {
...
func TestQueryNodesByFile_EmptyFromSQLite(t *testing.T) {
	db := tempSQLiteDB(t)
	ctx := context.Background()
	proj := "proj-empty"
	ver := "v1"
	...
	if len(nodes) != 0 {
		t.Errorf("expected empty result from SQLite, got %d nodes", len(nodes))
	}
}

func TestIngestCPGToSQLite_SinglePageDrain(t *testing.T) {
	db := tempSQLiteDB(t)
	ctx := context.Background()
	proj := "proj-drain"
	ver := "v1"
	...
	if err := g.IngestCPGToSQLite(ctx, db, proj, ver); err != nil {
