...
| **Token footprint per surface** | LLM received raw source code (hundreds to thousands of tokens per function) | Semantic Function Summarizer converts call chain context into ~50-token structured semantic abstractions | Order-of-magnitude token reduction per surface |
...
- **Built privacy-preserving LLM integration**: raw source code never reaches the LLM — a Semantic Function Summarizer converts code into structured security-relevant abstractions before any model call, aligned with IRIS (ICLR 2025)
